...
Процедура ПеренестиНоменклатуруВТабличнуюЧасть() Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПродажаБилета.Номенклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|	И НЕ ПродажаБилета.ПозицииПродажи.Количество() > 0";
	...
	Пока Выборка.Следующий() Цикл
		...
		Строка.Номенклатура = ДокОбъект.Номенклатура;
...
// Печатает билеты по переданному массиву документов "ПродажаБилета".
//
// Параметры:
//  МассивОбъектов - Массив - массив ссылок на документы "ПродажаБилета".
//  ОбъектыПечати - Массив - массив объектов печати.
//
// Возвращаемое значение:
//  ТабличныйДокумент - печатная форма билетов.
//
...
// Удаляет лидирующие нули из строкового представления номера.
//
// Параметры:
//  Номер - Строка - Строковое представление номера с возможными лидирующими нулями.
//
// Возвращаемое значение:
//  Строка - Строковое представление номера без лидирующих нулей.
//
